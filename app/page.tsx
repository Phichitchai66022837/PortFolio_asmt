...
function Page() {
  // ใช้ useEffect เพื่อลบ #About จาก URL เมื่อโหลดหน้าใหม่
  useEffect(() => {
    if (window.location.hash === '#About') {
      window.history.replaceState(null, '', window.location.pathname); // ลบ hash
    }
  }, []);
    ...
    <div className='bg-[url("../public/background.jpg")] w-screen h-screen object-cover bg-no-repeat bg-center bg-cover overflow-x-hidden relative scroll-smooth'>
      <nav className='flex justify-between pt-2 pr-10 pl-10 items-center h-[10%] animate-elementFade w-full sticky top-0  z-50 backdrop-opacity-50 backdrop-brightness-[0.1] border-b'>
        <div className='flex items-center'>
          <div className='size-7 cursor-pointer'>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill='white' className='hover:fill-gray-400 duration-[0.5s]'>
        ...
        <div>
          <ul className='flex font-semibold'>
            <li className='ml-7 cursor-pointer hover:text-gray-400 duration-[0.5s]'>
              <a href="#home">HOME</a>
            </li>
            <li className='ml-7 cursor-pointer hover:text-gray-400 duration-[0.5s]'>
      ...
      <main className='flex h-[90%] items-center mb-10'>
        ...
        <div className='w-[50%] flex justify-center items-center'>
          <div className='border-2 rounded-xl p-5 overflow-hidden'>
            <div className='bg-[url("./pic/riden.jpg")] object-cover bg-cover bg-center bg-no-repeat w-[350px] h-[550px] rounded-md animate-blinkingImg duration-1000 delay-[1000ms] hover:scale-125 hover:duration-100 hover:delay-100'></div>
          </div>
        </div>
      </main>
